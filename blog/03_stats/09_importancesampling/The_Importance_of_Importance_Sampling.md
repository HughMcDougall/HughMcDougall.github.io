...
# REDACT
Nsamp = 10_000
Xs = np.random.randn(Nsamp)

Xgrid  = np.linspace(-4,4,1024)
Xs = Xs[Xs.argsort()]
prior_old = lambda x: np.exp(-x**2/2 / 4)
...
plt.fill_between(logplot, logplot*P2.mean()/P1.mean(), P2.mean(), color='r', alpha=0.15, zorder=-np.inf)
